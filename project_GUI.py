...
class Window(Frame):
    ...
    def __init__(self, master=None):
        ...
        # function menu
        works = Menu(menu)
        works.add_command(label="Run Red  Light", command=self.choose_red_light)  # 0:Run Red  Light
        works.add_command(label="Speed Check", command=self.choose_speed)  # 1:Speed Check
        works.add_command(label="Vehicle Flowrate", command=self.choose_flowrate)  # 2:Vehicle Flowrate
        works.add_command(label="Zebra Crossing", command=self.choose_zebra)  # 3:Zebra Crossing
